...
<div id="col-right" class="col-right ">
	<div class="container-fluid primary-content">
		...
		<!-- SHOW HIDE COLUMNS -->
		<form action="alter_admin_form" accept-charset="utf-8" name="admin_form" method="get">
		<div class="widget">
			<h3 class="widget-header clearfix">
				...
				<div class="widget-content">
					<div class="table-responsive">
						<table style="border-right: #ccc 1px solid"
							...
							<c:choose>
								<c:when test="${fn:length(adminList) > 0}">
									<c:forEach items="${adminList}" var="row">
											<tr>
												<td><input type="radio" name="radio_button" value="${row.admin_no}"
												           class="checkSelect" required="required"></td>
												<td>${row.admin_no}</td>
												<td>${row.admin_name}</td>
												<td>${row.admin_pos}</td>
												<td>${row.admin_pnum}</td>
											</tr>
									</c:forEach>
								</c:when>
							</c:choose>
	...
	<div class="sticky-content pull-right" style="margin-top:10px; margin-right:30px">
						...
						<button type="button" onclick="location.href= '/manage_admin'"
